...
import { Box, Button, FormControl, FormLabel, Grid, MenuItem, Select, Stack, TextField, Typography } from "@mui/material";
...
export function AddTable() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors } } = useForm<TableType>({
    defaultValues: {
      family: 'ip',
      name: ''
    },
          ...
          <Stack spacing={3} width='70%' minWidth='600px'>
            ...
            <FormControl>
              ...
              <TextField
                {...register("name")}
              />
              {errors.name && errors.name.type === "required" && <Typography color='error'>{errors.name.message}</Typography>}
            </FormControl>
